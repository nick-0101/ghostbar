  ...
  <div class="ghostbar-inline-input-container" @mousedown="handleMouseDown">
    <div class="ghostbar-inline-input-container-inner Card">
      ...
      <div class="ghostbar-inline-input-actions-container">
        <Popover trigger="click" placement="top" :open="isPopoverOpen" @update:open="handlePopoverChange">
          <template #trigger>
            <button class="ghostbar-inline-input-model-selector Button" data-variant="ghost">Model: {{ userConversationsStore.selectedAiModel }}</button>
          </template>

          <div class="model-options">
            <button v-for="model in availableAiModels" :key="model" class="model-option" :class="{ active: userConversationsStore.selectedAiModel === model }" @click="userConversationsStore.selectedAiModel = model">
              {{ model }}
              <p>{}</p>
            </button>
          </div>
